...
std::vector<Segment> generate_lazy_roads(const Task& task, std::map<square_idx,Point> squares, const SolverSettings& stgs) {
    ...
    for (int64_t i = 0; i < stgs.squares_per_side; ++i) { // Цикл вдоль оси Y.
        for (int64_t j = 0; j < stgs.squares_per_side; ++j) { // Цикл вдоль оси X.
            ...
            // Проверяем ячейки справа, справа-сверху, сверху на доступность.
            for (auto near_square_idx : std::vector<square_idx>{{i,j+1}, {i+1,j+1}, {i+1,j}}) {
                // Пропускаем недоступные ячейки.
                if (!squares.contains(near_square_idx)) continue;

                auto& near_point = squares[near_square_idx];
                roads.emplace_back(curr_point, near_point);
            }
        }
    }
