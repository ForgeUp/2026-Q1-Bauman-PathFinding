...
std::vector<Qtree::Box*> Qtree::trace(const Segment& s) {
    ...
    if constexpr (upscale_on) {
        b1 = upscale(b1, p1);
        b2 = upscale(b2, p2);
    }
    
    if (b1 == b2) return {b1}; // Отрезок-точка или отрезок, вмещающийся в одну клетку.
    ...
    if (math::eq(p2.x - p1.x, 0)) { // Вертикальная линия.
        ...
        for (Point p = p1; !math::eq(p.y, p2.y, step); p.y += dy) {
            ...
            if constexpr (upscale_on) b = upscale(b, p);
            result.push_back(b);
        }
        return result;
    }
    ...
    while (b != b2) {
        ...
        Box* bq = locate(q);
        if (bq == nullptr) break;
        if constexpr (upscale_on) bq = upscale(bq, q);
        result.push_back(bq);
